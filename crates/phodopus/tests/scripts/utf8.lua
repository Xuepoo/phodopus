...
do
    ...
    local ab0c = "AB\0C"
    local ab0c_codes = collect_codes(ab0c)
    assert(#ab0c_codes == 4)
...
do
    ...
    assert(collect_codepoints("abc\xE2\x82", 1, -1) == false)
    assert(collect_codepoints("abc\xE2\x82\xFF", 1, -1) == false)
    assert(collect_codepoints("abc\xFF", 1, -1) == false)
...
do
    ...
    assert(res == nil and pos == 4)

    local res2, pos2 = utf8.len("abc\xE2\x82")
    assert(res2 == nil and pos2 == 4)

    local res3, pos3 = utf8.len("Привет", 2, 4)
    assert(res3 == nil and pos3 == 2)
end
