...
Phodopus is an independent, sandbox-first Lua runtime designed for uncompromising isolation, instruction Fuel budgeting, hard memory limits, and host-agnostic asynchronous execution. Fuel budgeting, stackless execution, the absorbed Phase 1/1.5 standard library and `load` work, the Phase 2 sandboxed module system (`require`, searcher chain, capability VFS), and the Phase 3 hard heap quota with OOM recovery (`RuntimeBuilder::memory_limit`, enforced at runtime-controlled allocation boundaries) are implemented; the async bridge remains target state. [Evolution Roadmap](architecture/roadmap.md) is the single implementation truth for phase status. Documentation drives implementation; every engineering task is guided by the specifications and architectural contracts contained in this corpus.
...
| [Sandbox & Fuel Specification](specifications/sandbox-and-fuel.md)         | Specification | Partial  | Deterministic instruction budgeting, preemption thresholds, and hard heap allocation ceilings (Fuel + quota enforcement).                                                                           |
...
| [Module Resolver Specification](specifications/module-resolver.md)         | Specification | Complete | Sandboxed `require` searcher chains, preloaded modules, and capability VFS resolvers.                                                                                                               |
...
| [Bitty Readiness Gate](integration/bitty-readiness-gate.md)                | Specification | Draft    | Per-item PASS/BLOCKED verdicts for RC-1/RC-2/RC-11, FS-1..FS-9, stdlib allowlists, diagnostics, module isolation, cancellation, and cross-platform evidence with the `bitty-lua` migration BLOCKED. |
