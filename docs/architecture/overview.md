...
- **Bounded `unsafe`**: `unsafe` is confined to specific VM and `gc-arena` primitives (raw table access, downcasting, string interning) plus system-level callback-erasure plumbing rather than eliminated. The callback erasure here is the runtime's internal typed VTable machinery, not the user-facing callback API (`Callback::from_fn`/`from_fn_with`), which is safe. The `phodopus-util/src/freeze.rs` lifetime-erasure support is a host utility, not a VM primitive. The exact inventory and its invariants are owned by the [Unsafe Code Ledger](../security/unsafe-ledger.md); the [threat model](../security/threat-model.md) defines the trust boundaries.
